import { useEffect, useRef, useState } from 'react'
import { Link, useNavigate } from 'react-router-dom'
import { User, ChevronDown, LogOut, Menu, Bookmark } from 'lucide-react'
import { useAuthStore } from '@/store/authStore'
import { getMyProfile } from '@/services/authService'
import { listWatchlists } from '@/services/watchlistService'
import type { Watchlist } from '@/types'
import NotificationBell from './NotificationBell'

interface HeaderProps {
  onMenuToggle: () => void
}

export default function Header({ onMenuToggle }: HeaderProps) {
  const { user, clearAuth } = useAuthStore()
  const navigate = useNavigate()

  const [displayName, setDisplayName] = useState<string | null>(null)
  const [dropdownOpen, setDropdownOpen] = useState(false)
  const [watchlistOpen, setWatchlistOpen] = useState(false)
  const [watchlists, setWatchlists] = useState<Watchlist[]>([])
  const dropdownRef = useRef<HTMLDivElement>(null)
  const watchlistRef = useRef<HTMLDivElement>(null)

  const showWatchlist = user?.userType === 'CLIENT' || user?.userType === 'EMPLOYEE'

  useEffect(() => {
    if (!user) return
    getMyProfile()
      .then((p) => setDisplayName(`${p.first_name} ${p.last_name}`.trim()))
      .catch(() => setDisplayName(user.email))
  }, [user?.email])

  useEffect(() => {
    function handleClick(e: MouseEvent) {
      if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) {
        setDropdownOpen(false)
      }
      if (watchlistRef.current && !watchlistRef.current.contains(e.target as Node)) {
        setWatchlistOpen(false)
      }
    }
    document.addEventListener('mousedown', handleClick)
    return () => document.removeEventListener('mousedown', handleClick)
  }, [])

  async function handleWatchlistOpen() {
    if (watchlistOpen) { setWatchlistOpen(false); return }
    setWatchlistOpen(true)
    try {
      const lists = await listWatchlists()
      setWatchlists(lists)
    } catch {
      setWatchlists([])
    }
  }

  function handleLogout() {
    clearAuth()
    navigate('/login', { replace: true })
  }

  return (
    <header className="h-14 flex items-center justify-between px-4 sm:px-6 bg-white border-b border-gray-200 shrink-0">
      {/* Left: hamburger for mobile */}
      <button
        onClick={onMenuToggle}
        className="md:hidden p-2 rounded-lg text-gray-500 hover:text-gray-800 hover:bg-gray-100 transition-colors"
        aria-label="Otvori meni"
      >
        <Menu className="h-5 w-5" />
      </button>

      {/* Spacer for desktop */}
      <div className="hidden md:block" />

      {/* Right: watchlist + notifications + user info */}
      <div className="flex items-center gap-1">

        {/* Watchlist quick access */}
        {showWatchlist && (
          <div className="relative" ref={watchlistRef}>
            <button
              onClick={handleWatchlistOpen}
              className="p-2 rounded-lg text-gray-500 hover:text-gray-800 hover:bg-gray-100 transition-colors"
              title="Watchliste"
            >
              <Bookmark className="h-5 w-5" />
            </button>

            {watchlistOpen && (
              <div className="absolute right-0 mt-1 w-56 rounded-xl border border-gray-200 bg-white shadow-lg py-1 z-50">
                <div className="px-3 py-2 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wide">
                  Watchliste
                </div>
                {watchlists.length === 0 ? (
                  <p className="px-4 py-3 text-sm text-gray-400">Nemate watchlisti.</p>
                ) : (
                  watchlists.slice(0, 5).map((wl) => (
                    <Link
                      key={wl.id}
                      to={`/watchlist`}
                      onClick={() => setWatchlistOpen(false)}
                      className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
                    >
                      <Bookmark className="h-4 w-4 text-gray-400 flex-shrink-0" />
                      <span className="truncate">{wl.name}</span>
                    </Link>
                  ))
                )}
                <div className="border-t border-gray-100 mt-1">
                  <Link
                    to="/watchlist"
                    onClick={() => setWatchlistOpen(false)}
                    className="flex w-full items-center px-4 py-2.5 text-sm text-primary-600 hover:bg-gray-50 font-medium transition-colors"
                  >
                    Sve watchliste →
                  </Link>
                </div>
              </div>
            )}
          </div>
        )}

        <NotificationBell />

        {/* User dropdown */}
        <div className="relative" ref={dropdownRef}>
          <button
            onClick={() => setDropdownOpen((v) => !v)}
            className="flex items-center gap-2.5 rounded-lg px-3 py-1.5 hover:bg-gray-100 transition-colors"
          >
            <div className="flex items-center justify-center h-8 w-8 rounded-full bg-primary-100 text-primary-700 shrink-0">
              <User className="h-4 w-4" />
            </div>
            <span className="text-sm font-medium text-gray-800 max-w-[160px] truncate">
              {displayName ?? user?.email ?? '…'}
            </span>
            <ChevronDown
              className={[
                'h-4 w-4 text-gray-400 transition-transform duration-150',
                dropdownOpen ? 'rotate-180' : '',
              ].join(' ')}
            />
          </button>

          {dropdownOpen && (
            <div className="absolute right-0 mt-1 w-44 rounded-xl border border-gray-200 bg-white shadow-lg py-1 z-50">
              <button
                onClick={handleLogout}
                className="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
              >
                <LogOut className="h-4 w-4 text-gray-400" />
                Odjavi se
              </button>
            </div>
          )}
        </div>
      </div>
    </header>
  )
}
